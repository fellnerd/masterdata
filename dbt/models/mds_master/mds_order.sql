      ...
      WHERE is_current = 1 
        AND business_key IN (
          ...
          WHERE is_processed = 0 
            ...
            AND business_key IN (SELECT business_key FROM mds_master.[order] WHERE is_current = 1)
        )
      {% endif %}"
    ],
    post_hook=[
      "-- Mark load records as processed",
      "UPDATE mds_load.[order] SET is_processed = 1, processed_at = GETUTCDATE() WHERE is_processed = 0",
      ...
      "DELETE FROM mds_load.[order] WHERE operation = 'DELETE'"
    ]
  )
}}
...
{#
  ...
  Entity Code: order
  Generated:   2026-01-12T12:29:45.557033
  
  Source: mds_load.[order]
  Target: mds_master.[order] (SCD2 historisiert)
    ...
    SELECT 
        ...
        CASE 
            ...
            WHEN s.operation = 'UPDATE' OR (
                COALESCE(CAST(s.order_id AS NVARCHAR(MAX)), '') != COALESCE(CAST(t.order_id AS NVARCHAR(MAX)), '') OR
