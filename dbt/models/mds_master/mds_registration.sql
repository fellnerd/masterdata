{{
  config(
    ...
    pre_hook=[
      ...
      UPDATE mds_master.registration
      ...
      WHERE is_current = 1 
        AND business_key IN (
          SELECT business_key 
          FROM mds_load.registration 
          WHERE is_processed = 0 
            AND operation IN ('UPDATE', 'DELETE', 'INSERT')
            AND business_key IN (SELECT business_key FROM mds_master.registration WHERE is_current = 1)
        )
      {% endif %}"
    ],
    post_hook=[
      "-- Mark load records as processed",
      "UPDATE mds_load.registration SET is_processed = 1, processed_at = GETUTCDATE() WHERE is_processed = 0",
      "-- Update commit status to 'deployed' for all loaded commits",
      "UPDATE mds_stage.[commit] SET status = 'deployed' WHERE status = 'loaded' AND entity_id = 1{% if var('deploy_commit_ids', none) %} AND id IN ({{ var('deploy_commit_ids')|join(',') }}){% endif %}",
      "-- Remove DELETE records from load (they should not appear in current state)",
      "DELETE FROM mds_load.registration WHERE operation = 'DELETE'"
    ]
  )
}}
...
{#
  =====================================================
  MDS Master: Registration
  =====================================================
  
  Entity Code: registration
  Generated:   2026-07-29T18:45:58.856500
  
  Source: mds_load.registration
  Target: mds_master.registration (SCD2 historisiert)
  
  Business Key: name
  Columns: name, value, embedded_url
  =====================================================
#}
    ...
    SELECT 
        ...
        CASE 
            ...
            WHEN s.operation = 'UPDATE' OR (
                COALESCE(CAST(s.name AS NVARCHAR(MAX)), '') != COALESCE(CAST(t.name AS NVARCHAR(MAX)), '') OR
                COALESCE(CAST(s.value AS NVARCHAR(MAX)), '') != COALESCE(CAST(t.value AS NVARCHAR(MAX)), '') OR
                COALESCE(CAST(s.embedded_url AS NVARCHAR(MAX)), '') != COALESCE(CAST(t.embedded_url AS NVARCHAR(MAX)), '')
            ) THEN 'CHANGED'
            ELSE 'NO_CHANGE'
        END AS change_type
    FROM source_data s
