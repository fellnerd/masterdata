  ...
  config(
    ...
    pre_hook=[
      ...
      WHERE is_current = 1 
        AND business_key IN (
          SELECT business_key 
          FROM mds_load.test_entity 
          WHERE is_processed = 0 
            AND operation IN ('UPDATE', 'DELETE', 'INSERT')
            AND business_key IN (SELECT business_key FROM mds_master.test_entity WHERE is_current = 1)
        )
      {% endif %}"
    ],
    post_hook=[
      "-- Mark load records as processed",
      "UPDATE mds_load.test_entity SET is_processed = 1, processed_at = GETUTCDATE() WHERE is_processed = 0",
      "-- Update commit status to 'deployed' for all loaded commits",
      "UPDATE mds_stage.[commit] SET status = 'deployed' WHERE status = 'loaded' AND entity_id = 3",
      "-- Remove DELETE records from load (they should not appear in current state)",
      "DELETE FROM mds_load.test_entity WHERE operation = 'DELETE'"
    ]
  )
}}
...
{#
  =====================================================
  MDS Master: Test Entity
  =====================================================
  
  Entity Code: test_entity
  Generated:   2026-01-14T19:26:18.368523
  
  Source: mds_load.test_entity
  Target: mds_master.test_entity (SCD2 historisiert)
    ...
    SELECT 
        ...
        CASE 
            ...
            WHEN s.operation = 'UPDATE' OR (
                COALESCE(CAST(s.test_id AS NVARCHAR(MAX)), '') != COALESCE(CAST(t.test_id AS NVARCHAR(MAX)), '') OR
                COALESCE(CAST(s.test_name AS NVARCHAR(MAX)), '') != COALESCE(CAST(t.test_name AS NVARCHAR(MAX)), '')
            ) THEN 'CHANGED'
            ELSE 'NO_CHANGE'
        END AS change_type
    FROM source_data s
